...
namespace WoWonder.Activities.Live.Page
{
    ...
    public class RtcBaseActivity : AppCompatActivity
    {
        ...
        protected AgoraRender AgoraRender;
        protected BackgroundRender BackgroundRender;
        ...
        private void InitToken()
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(ListUtils.SettingsSiteList?.AgoraAppCertificate))
                {
                    ...
                    token = RtcTokenBuilder.BuildTokenWithUid(AppSettings.AppIdAgoraLive, ListUtils.SettingsSiteList?.AgoraChatAppCertificate, channelName, uid, RtcTokenBuilder.Role.RolePublisher, timestamp);
                }
            }
            catch (Exception e)
            {
        ...
        private void InitRtcEngine()
        {
            try
            {
                ...
                config.MAreaCode = RtcEngineConfig.AreaCode.AreaCodeGlob;
        ...
        protected void JoinChannel()
        {
            try
            { 
                ChannelMediaOptions option = new ChannelMediaOptions((Integer)Constants.ClientRoleBroadcaster);
        ...
        private void InitExtension()
        {
            try
            {
                if (AgoraRender == null)
                {
                    AgoraRender = new AgoraRender(this, MRtcEngine);
                }

                if (BackgroundRender == null)
                {
                    BackgroundRender = new BackgroundRender(this, MRtcEngine);
                }
            }
            catch (Exception e)
            {
