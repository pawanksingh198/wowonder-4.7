...
namespace WoWonder.Activities.Market
{
    ...
    public class EditProductActivity : BaseActivity, View.IOnFocusChangeListener, IDialogListCallBack
    {
        ...
        private void AddOrRemoveEvent(bool addEvent)
        {
            try
            {
                switch (addEvent)
                {
                    // true +=  // false -=
                    case true:
                        MAdapter.ItemEditClick += MAdapterOnItemEditClick;
        ...
        private void MAdapterOnItemEditClick(object sender, AttachmentsAdapterClickEventArgs e)
        {
            try
            {
                if (e.Position >= 0)
                {
                    ...
                    if (item.TypeAttachment.Contains("postPhotos"))
                    {
                        IdImage = item.Id;
        ...
        protected override void OnActivityResult(int requestCode, Result resultCode, Intent data)
        {
            try
            {
                ...
                else if (requestCode == 3000 && resultCode == Result.Ok)
                {
                    ...
                    {
                        var change = MAdapter.AttachmentList.FirstOrDefault(q => q.Id == IdImage);
                        if (change != null)
                        {
                            change.FileUrl = imagePath;
                            change.FileSimple = imagePath;

                            MAdapter.NotifyItemChanged(MAdapter.AttachmentList.IndexOf(change));
                        }
                    }
                }
            }
            ...
            try
            {
                ...
                switch (requestCode)
                {
                    case 108 when grantResults.Length > 0 && grantResults[0] == Permission.Granted:
                        PixImagePickerUtils.OpenDialogGallery(this, false, true);
                        break;
                    case 108:
